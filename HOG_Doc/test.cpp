...
void Lanes::Brightest_Pixel()
{
	...
	for(int i = 0; i < top_view.cols; i++)  
	{  // finding the brightest pixel in each row
		int l,r,max = 0;
		for(int j = 0; j < top_view.rows/2; j++)
		{
			...
			for(int m = i-1; m < i+2; m++)
				for(int n = j-1; n < j+2; n++)
				{
					if(m < 0 || n < 0 || m >= top_view.cols || n >= top_view.rows) continue;
		...
		if(max>150) bisect.at<uchar>(l,i) = 255;
		max = 0;
		for(int j = top_view.rows/2 + 1; j < top_view.rows; j++)
		{
			//bisect.at<uchar>(j,i) = 0;
			// if(img_gray.at<uchar>(i,j) < INTENSITY_TH) img_gray.at<uchar>(i,j) = 0;
			// else img_gray.at<uchar>(i,j) = 255;
			...
			for(int m = i-1; m < i+2; m++)
				for(int n = j-1; n < j+2; n++)
				{
					if(m < 0 || n < 0 || m >= img.cols || n >= img.rows) continue;
					sum += top_view.at<uchar>(m,n); count++;
				}
			if(count != 0) sum /= count;
			if(sum > max)
		...
		if(max > 150) bisect.at<uchar>(r,i) = 255;
	}
	// Dilation();
